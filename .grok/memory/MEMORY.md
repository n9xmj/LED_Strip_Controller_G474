...
- [Decision-log planning model](planning_decision_log_model.md) — **The Big Board** = summary table (D/S/I/T/Q, 🔴🟡🟢🔵) + detail sections; resolve in chat by label (*"green D3"*). Full doc: `Docs/planning/decision-log-model.md`. **Active plan:** `Docs/planning/play-v1-implementation-plan.md`. **Handoff:** newest `Docs/planning/play-v1-session-handoff-*.md` (via `/wrapup`). **Account switch (2026-06-13):** `Docs/planning/play-v1-account-switch-handoff-2026-06-13.md`. **Focused sessions:** `Docs/planning/focused-implementation-handoff-template.md`; `/cleanup-docs` after MSG ✅.
