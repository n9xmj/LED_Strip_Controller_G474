#!/usr/bin/env python3
"""
term line-editor bench (T4) — drive the G474 test-harness REPL to run scripted
edit sessions via the 'E <hex>' op and assert exit code + resulting line.

Usage:
  python scripts/term_lineedit_bench.py run
  python scripts/term_lineedit_bench.py run type_hello mid_space
  python scripts/term_lineedit_bench.py list
...
GOLDEN_LINEEDIT = REPO_ROOT / "scripts" / "term_golden" / "lineedit.json"

HARNESS_ENTER = b"\xDA"
HARNESS_EXIT = b"\xA5"
LINE_RE = re.compile(r'<HRN E rc=(\d+) line="((?:\\.|[^"\\])*)">')


def load_vectors() -> dict:
    if not GOLDEN_LINEEDIT.is_file():
        return {}
    with GOLDEN_LINEEDIT.open(encoding="utf-8") as fh:
        data = json.load(fh)
...
class HarnessClient:
    ...
    def lineedit(self, hex_stream: str, timeout_s: float = 3.0) -> tuple[str | None, str | None]:
        self._write(f"E {hex_stream}\r".encode("ascii"))
        text = self._read_until("<HRN E ", timeout_s)
...
def cmd_list(_: argparse.Namespace) -> int:
    ...
    print(f"Golden line-edit vectors ({GOLDEN_LINEEDIT}):\n")
    for name, v in vectors.items():
        print(f"  {name:<16} rc={v['expect_rc']:<3} line={v['expect_line']!r:<16} send={v['send']}")
    return 0
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for name in selected:
            v = vectors[name]
            got_rc, got_line = client.lineedit(v["send"])
            ok = (got_rc == v["expect_rc"]) and (got_line == v["expect_line"])
            mark = "PASS" if ok else "FAIL"
            print(f"  [{mark}] {name:<16} want rc={v['expect_rc']} line={v['expect_line']!r} "
    ...
    total = len(selected)
    print(f"\n=== term line-edit bench: {npass}/{total} passed ===")
    return 1 if failures else 0


def build_parser() -> argparse.ArgumentParser:
    defaults = load_bench_defaults()
    p = argparse.ArgumentParser(description="term line-editor bench (harness E op)")
