...
void v_job_add(job_queue_t *p_x_queue, uint8_t u8_job_id)
{
    ...
    DISABLE_INTERRUPTS(u32_primask);
...
#if JOB_QUEUE_PROVIDE_POINTER_PARAM
        p_x_job->p_v_pointer = NULL;
#endif
        p_x_queue->u8_head++;
    ...
    RESTORE_INTERRUPTS(u32_primask);
}
...
void v_job_add_with_pointer(job_queue_t *p_x_queue, uint8_t u8_job_id, uint8_t u8_param1, uint16_t u16_param2, void *p_v_pointer)
{
    if (p_x_queue == NULL)
    {
    #if DEFAULT_JOB_QUEUE_SIZE > 0
    ...
    if (! p_x_queue->u8_full)
    {
        ...
        p_x_job->u8_param1 = u8_param1;
        p_x_job->u16_param2 = u16_param2;
        p_x_job->p_v_pointer = p_v_pointer;
...
void v_job_add_with_pointer(job_queue_t *p_x_queue, uint8_t u8_job_id, uint8_t u8_param1, uint16_t u16_param2, void *p_v_pointer)
{
    // Pointers in job queue not enabled
    // Add a job with only u8_param1 and u16_param2; p_v_pointer ignored/discarded
    (void) p_v_pointer; // Reference this unused param to avoid compiler warning
    v_job_add_with_params(p_x_queue, u8_job_id, u8_param1, u16_param2);
}
#endif
...
bool b_job_queue_empty(job_queue_t *p_x_queue)
{
    if (p_x_queue == NULL)
    {
#if DEFAULT_JOB_QUEUE_SIZE > 0
    ...
    bool b_queue_empty = true;
    uint32_t u32_primask;
    DISABLE_INTERRUPTS(u32_primask);
    b_queue_empty = (p_x_queue->u8_head == p_x_queue->u8_tail) && !p_x_queue->u8_full;
    RESTORE_INTERRUPTS(u32_primask);

    return b_queue_empty;
}
