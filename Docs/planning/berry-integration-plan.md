...
| W3 | 🟢 | **Filesystem tie-in via C stdio — CORE DONE + HIL-verified 2026-07-04.** Berry `open()/read()/write()/seek()/size()/close()` now route through libc stdio → the label-routed littlefs VFS (spiflash W10/W12 Phase B), so `open("/lfs0/x.be")` works from scripts. **As built:** `BE_USE_FILE_SYSTEM 0→1` (`berry_conf.h`); the `default/be_port.c` file-op stubs wired to `fopen/fclose/fwrite/fread/fgets/fseek(SEEK_SET)/ftell/fflush/fsize`, preserving the stdin/stdout/stderr console sentinels (file writes stay byte-exact — LF→CRLF lives only in `be_writebuffer`, the print path). **coc regen turned out to be a NO-OP:** `open` is registered *unconditionally* in the base `m_builtin` (`src/be_baselib.c`) and the `file` class is built at runtime in `be_nfunc_open` (`src/be_filelib.c`), so nothing is gated on `BE_USE_FILE_SYSTEM` once `os` stays off — coc rewrote all 23 generated headers byte-identical (the plan's "build gate" assumption was conservative). Dir-traversal (`be_isdir`/`be_dirfirst`) + the `os` module stay off (follow-on). **Verified** through the new `Y` harness op (see W4): `scripts/spiflash_bench.py --suite berry` (9 checks — write→reopen→read round-trip on a scratch `@tr_` partition, `size()`, failure-rc plumbing). **Per-VM cwd layer DONE 2026-07-04:** script globals `chdir(path)` + `getcwd()` (registered via `be_regfunc` at VM creation — the native seam in `berry_app.c`), cwd stored **per-VM** in the session struct (empty on a fresh VM). `be_fopen` resolves a relative path against the active cwd (absolute `/…` bypasses; littlefs handles `.`/`..` below the label; default-empty cwd means relative opens fail until `chdir`). Reached through a global active-cwd pointer swapped on session open/close behind the `pc_berry_active_cwd()` accessor — **same mechanism + RTOS caveat as the W8 arena**, and commented as the shared **RTOS migration point** (goes thread-local/per-task under a preemptive RTOS; cooperative interleaving only needs scheduler-level swapping). Verified: `--suite berry` (11 checks) incl. `cwd_relative_open` + `cwd_fresh_vm_empty` (proves per-VM isolation). Dir-normalization / cross-label `..` remain the Tier-2 VFS wishlist. **Distinct from** spiflash-plan W7 (flash-specific script API) **and** Berry W1 (PLAY-as-a-func). |
| W4 | 🔵 | **Test-runner single-instance Berry:** external host script uploads a Berry script into RAM; one persistent embedded VM runs it to completion and exits. Realized via **I4's headless run front-end** (`be_loadbuffer` → `be_pcall`, already stubbed as `i_berry_run_buffer`). Depends on the cooperative pump (S2) to keep the polling task / job queue alive during the run. **Planned addition (2026-06-26, next session):** a generalized "file upload" — a means for a test/host to load arbitrary data into a static or malloc'd RAM buffer for use by *other* tests, **including** handing that buffer to a Berry VM to execute. Generalizes the upload-script-into-RAM path to arbitrary data buffers; builds on `i_berry_run_buffer(ptr,len)` + the W8 arena. **Harness front door DONE 2026-07-04:** the **`Y` op** in [`test_harness.c`](../../App/Src/test_harness.c) hex-decodes a script into a static RAM buffer and calls `i_berry_run_buffer`, framing `<HRN Y done rc=N len=M>` — automated Berry runs with **no line editor** (reuses the harness `x_harness_read_line` raw reader; this is the W3 verification path). *Still open:* the generalized arbitrary-blob upload + a length-prefixed **binary packet transport** (vs today's hex-in-text) for the W13 FS-shell get/put — a W9/W13 transport evolution. |
