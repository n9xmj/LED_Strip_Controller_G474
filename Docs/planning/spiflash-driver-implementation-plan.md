...
| I5 | 🟢 | **Partition system — v1 scope (locked)** (promoted from W1). ESP-*conceptual*; table in **sector 0** (magic+ver+CRC32 + entry array). Default layout: `0`=table, `1–3`=generic data (stress test), `4–5`=nvmparams, `6–9`=reserved-A, `10..n-2`=two **equal** littlefs, `n-1..n`=reserved-B. Fixed regions = 12 sectors (even) → littlefs `N-12` splits exactly, no gap. See detail table. |
...
| G7 | 4 | ✅ | **littlefs BD shim** — `App/spiflash/spiflash_lfs.{c,h}`: `read/prog/erase/sync` callbacks (`cfg.context` → the bound FS, ops via `x_spiflash_part_*`) + `lfs_config` from runtime geometry (prog=page, block=sector, count=part/block). `spiflash_lfs_t` bundles `lfs_t`+config+static caches+partition; instantiated **twice**. littlefs un-excluded from the build (lfs.c/lfs_util.c compile in). **HIL-validated via G8 (2026-06-28).** | D2, I3, I5 |
...
| W7 | 🔵 | **Berry flash/FS script API** — expose the flash + partition + VFS ops as Berry native functions/module, **layered on** the Berry FS-via-stdio tie-in (**Berry plan W3**). *Distinct from* PLAY-as-a-Berry-function (Berry plan **W1**, the synth tie-in) *and* from the raw stdio wiring itself (**Berry plan W3**). |
...
| W13 | 🔵 | **Host-side filesystem shell (interactive REPL)** — a Python shell over the device host-protocol giving reasonably complete remote FS access: `ls` / `cd` / `pwd`, `rm`, `rename`, `cp` / `mv`, plus `get` (copy-to-host) and `put` (upload-from-host). Unix-like, using the W12 `/<label>/...` path scheme across both littlefs partitions. The host-side **front-end** over the **W9** device fs ops (W9 = the primitives; W13 = the usable shell). Depends on W9 + G7/G8 (+ W12 paths). **Active decision-log plan:** [host-fs-shell-plan.md](host-fs-shell-plan.md) (2026-07-14). See detail. |
...
### I5 — Partition system
**Status:** 🟢 · **Needs user:** no (locked 2026-06-27; v1 scope, promoted from W1)
...
**Resolution (locked):** layout + on-flash format above are fixed; the concrete table-header field
widths and the provisioning routine are implemented at G11 (no design questions remain).
...
**Refactor scope (W8 also does this):** a *partial refactor* of nvmparams for tighter **encapsulation**
(clean module boundary, no project-global state leakage) and **portability** (keep the `nvm_device_t`
read/write switch the single platform touchpoint). The reusable engine/API separates cleanly from the
project-specific slot-ID set.
**Relationship to G13:** W8 is an eventual **follow-up to the boot-time init (G13)** but **not a G13
requirement** — it stands on its own and can land on its own schedule. Division of labour: **G13 mounts
`LITTLEFS`-type partitions; NVM-type partitions are brought up here, by nvmparams init.** W8 finds its
region by **`SPIFLASH_PART_TYPE_NVM`** (or label) — same type-driven discovery as G13.
...
**Status:** 🔵 · **Needs user:** yes (UX dialect — see dedicated plan)  
**Decision-log plan:** [host-fs-shell-plan.md](host-fs-shell-plan.md) — Big Board, MSG, wish list, command/alias table. Prefer that doc for locks; keep this row as the spiflash-roadmap pointer.
**What:** a host-side Python **interactive shell** exposing the device littlefs FS with familiar unix
*and* DOS-style aliases — `ls`/`dir`, `cd`/`pwd`, `rm`/`del`, `rename`, `cp`/`mv`, plus host↔device
transfer: `get`/`download` and `put`/`upload`. Classic line REPL (edit + history); not a dual-pane TUI.
Paths are unix-like and **label-qualified per W12** (`/<partition-label>/dir/file`); dual host/remote
cwd with world flags (`-L`/`--local`, default remote).
