...
| **W17** | **Field display attributes (SGR `ESC[…m`)** — caller-selectable rendition (reverse / underline / bold / dim, etc.) applied to the **entry field text** in both modes (most useful for bounded fixed-width fields, e.g. a highlighted form field). Opt-in via `term_line_edit_t` (mirrors the **W12 `b_show_mode_cursor`** opt-in pattern): editor wraps every painted cell run in the chosen `ANSI_*` attribute(s) and emits `ANSI_ATTR_OFF` (and restores on every exit path, the **I8/D13** restore discipline) so attributes never leak past the field. Reuses existing `ANSI.h` SGR macros via new `v_term_*` wrappers (**W14** family). **Stretch:** also accept ANSI **foreground/background colors** (256-color + RGB macros already in `ANSI.h`). Open Qs for when it's picked up: how to pass the attribute set (bitmask of named attrs vs a small caller-provided SGR string), and whether the prompt is styled too or just the field. |
