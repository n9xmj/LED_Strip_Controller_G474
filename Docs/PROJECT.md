...
- [ ] **I2C OLED + rotary-encoder control module.** Cheap hobbyist board: a small I2C OLED (the usual jellybean controller — almost certainly **SSD1306**, possibly **SH1106**; confirm against the module before writing a driver, they differ in column offset and addressing) plus a rotary encoder and a few pushbuttons. **Spadework only so far:** TIM5 is allocated in Encoder_Interface mode and `MX_TIM5_Init()` runs, but **nothing in the application reads it** — no encoder API, no button handling, no display driver. I2C1 was already provisioned, so the display bus exists.
  - Pins: **`ENCODER_A` = PB2** (TIM5_CH1), **`ENCODER_B` = PA1** (TIM5_CH2). TIM5 is 32-bit on the G4, so the count is `Period = 0xFFFFFFFF` and wrap handling is a non-issue in practice.
  - **Two generated-config values will need revisiting once real hardware is attached**, and they are the usual causes of a rotary encoder that reads erratically:
    - `IC1Filter` / `IC2Filter` are **0** — no input filter. Mechanical encoders bounce; the timer's input filter is the standard cure and costs nothing. Expect to raise this.
    - `EncoderMode` is **`TIM_ENCODERMODE_TI1`** (x2, counting edges on CH1 only). A detented encoder usually wants x4 (`TI12`) with a divide, or x2 aligned to the detent — pick after observing counts per detent on the actual part.
  - Buttons: not allocated yet. Decide EXTI vs polled with the existing job/polling loop.
  - Whole thing is a new-session task; it touches no existing subsystem.
