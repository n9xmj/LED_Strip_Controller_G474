...
- [x] **10.** Integrate legacy logging API. Split performed: sugar macros in `App/logging-api/log_helpers.h`, project tags (LOG_SYSTEM/LED/I2S_OUT/I2S_IN) + single-include `debug_config.h` in `App/Inc/`. Core in logging.h + logging.c (G4 ported). RPRINTF used for banner. Portable modules stay clean. (Further LOGCT adoption in app code is ongoing.)
